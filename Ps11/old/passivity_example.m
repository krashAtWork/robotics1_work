...
Bt_mat = [Bt,-Bt;-Bt,Bt];
Kt_mat = [Kt,-Kt;-Kt,Kt];
B_actuator = 50; %actuator has lots of friction; want to mask this with feedback



Kenv= 200 %100 %120 %200000 %environment model: a spring of this stiffness, N/m

%desired impedance of Bdes and Kdes
%LOWER BDES IS BETTER, implies more responsive closed-loop admittance of the robot
Bdes = 1 %damping of desired admittance
Kdes = 0; %10; %20; %100 %stiffness of desired admittance



%servo controller gains:
Kp = 100%200 %200 %20000  %200
Kv = 40 %40 %3000  %40
...
for k=1:npts
  w=freq_range(k);
    s = 1i*w;
   Z_plant = M*s +Bt_mat + [1;0]*B_actuator*[0,1] +Kt_mat/s; % +Fdbk_mat;   
   B_plant = [0;1];
...
for k=1:npts
  ...
  servo_fdbk_mat = [Kp/s+Kv,0;0,0];
   ...
   %Gv = (exp(-s*T_D)/(s^2))*(Kp)*J_inv*inv(Bvirt)*(Kvirt)*J + (Kv + Kp/s);
    ...
    LHS_mat= Z_plant + [1;0]*(Gv)*[1,0];
    RHS_vec = [0;1]+[Gf;0];    %these terms multiply Fe on the right-hand side of the eqns
...
subplot(2,1,1)
loglog(freq_range, Ycl_mag_pts,'r')
ylabel('Y magnitude')
grid on
hold on
title('robot admittance: des (b), realized (r)')


subplot(2,1,2)
semilogx(freq_range,Ycl_phase_pts,'r')
xlabel('freq (rad/sec)')
ylabel('phase (rad)')
grid on
hold on
...
semilogx(freq_range,Ze_phase_pts)
xlabel('freq (rad/sec)')
ylabel('phase (rad)')
hold on 
...
for k=1:npts
  w=freq_range(k);
   ...
   mag_pts(k) = abs(YclZenv);
   phase_pts(k) = atan2(imag(YclZenv),real(YclZenv));
